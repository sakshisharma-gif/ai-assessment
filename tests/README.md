...
- [Test Types](#test-types)
  - [State Machine Integration Tests ★](#4-state-machine-integration-tests--mandatory-tier)
...
The backend test suites live under **`src/backend/__tests__/`** and are executed
with the backend's own scripts. **All `npm test` / `npx jest` commands in this
guide must be run from the `src/backend` directory:**
...
The mandatory **status state-machine** integration tests are located at:

```
src/backend/__tests__/integration/stateMachine.test.js
```
...
### 4. State Machine Integration Tests ★ Mandatory Tier

**File:** `src/backend/__tests__/integration/stateMachine.test.js`
...
> Related unit-level checks live in `models/Ticket.test.js` (the
> `Status Transition Logic` describe block), which tests
> `ticket.canTransitionTo(newStatus)` directly.

#### Step-by-step: running the state-machine tests

```bash
# 1. Move into the backend package (tests run from here)
cd src/backend

# 2. Install dependencies (first time only)
npm install

# 3. Run ONLY the state-machine integration suite
npx jest __tests__/integration/stateMachine.test.js

# 4. See each transition case as it runs
npx jest __tests__/integration/stateMachine.test.js --verbose

# 5. Run just the valid-transition cases
npx jest __tests__/integration/stateMachine.test.js -t "Valid transitions succeed"

# 6. Run just the invalid/rejected-transition cases
npx jest __tests__/integration/stateMachine.test.js -t "Invalid transitions are rejected"
```

**Expected result:** `Test Suites: 1 passed`, `Tests: 38 passed`.

A summary of the latest run is kept in the repository-root `test-results.md`.
...
For developers new to this testing setup (run from `src/backend`):

```bash
# 0. Move into the backend package
cd src/backend
